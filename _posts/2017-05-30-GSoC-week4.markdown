...
The past week involved creating the new module for my exploration world and getting started with it.
...
## Integrating Journal
A key part of the new exploration world would be a journal that links several things. The main purpose of the Journal would be to serve as a secondary memory for the player. I plan to use the Journal in several ways-
...
I've added another enhancement that makes the Journal more interactive. Previously, a notification would pop up when a new Journal Entry was added, but it wouldn't be highlighted in any way within the Journal itself. With the [latest PR](https://github.com/Terasology/Journal/pull/3/) merged, unread Journal entries now appear highlighted in yellow until their chapter gets opened once.
...
+ If the entity also has a `PlayerCharacterComponent`, the event is consumed to prevent destruction. The `AliveCharacterComponent` is stripped off and a `PlayerDeathEvent` sent along with `DeathEvent`
